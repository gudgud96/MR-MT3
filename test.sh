...
python3 test.py \
    ...
    --config-name="config" \
    model="MT3Net" \
    path="/workspace/data/kinwai/mt3/pretrained/mt3.pth" \
    eval.eval_dataset="Slakh" \
    eval.exp_tag_name="slakh_mt3_official" \
    eval.audio_dir="/workspace/data/dataset/slakh2100_flac_redux/test/*/mix_16k.wav" \
    hydra/job_logging=disabled \
    eval.is_sanity_check=True   \
    eval.eval_first_n_examples=9999 \
