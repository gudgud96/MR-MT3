...
from models.transformer_segmem_v2_with_prev import VanillaTransformerSegMemV2WithPrev
from utils import get_cosine_schedule_with_warmup
from tasks.mt3_base import MT3Base


class VanillaTransformerNetSegMemV2WithPrev(MT3Base):
    def __init__(self, config, optim_cfg, eval_cfg=None):
        super().__init__(config, optim_cfg, eval_cfg=eval_cfg)
        self.model: nn.Module = VanillaTransformerSegMemV2WithPrev(config=config)
