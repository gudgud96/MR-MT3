...
class T5DETR(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        self.decoder = T5StackDETR(decoder_config, self.decoder_embed_tokens)
...
class T5StackDETR(T5PreTrainedModel): # Modified from T5Stack to be a DETR decoder
    def __init__(self, config, embed_tokens=None):
        super().__init__(config)
        ...
        # query tokens for DETR
        self.query_embed = nn.Embedding(config.detr_num_tokens, config.detr_hidden_dim)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            inputs_embeds = self.query_embed.weight
            # repeat inputs dimension to be the same as batch size
        ...
        # DETR is not autoregressive, so we do not need to mask the future
        # mask_seq_length = past_key_values[0][0].shape[2] + \
        #     seq_length if past_key_values is not None else seq_length
        # past_key_values_length = past_key_values[0][0].shape[2] if past_key_values is not None else 0
        ...
        if self.is_decoder and encoder_attention_mask is None and encoder_hidden_states is not None:
            ...
            encoder_attention_mask = torch.ones(
                batch_size, encoder_seq_length, device=inputs_embeds.device, dtype=torch.long
            ) # if this mask is all one, why bother?

        # initialize past_key_values with `None` if past does not exist
        # TODO: remove this portion, it won't be used in DETR
        if past_key_values is None:
            past_key_values = [None] * len(self.block)
        ...
        # pos_emb
        # DETR does not use positional embedding for the queries
        # It learns the position embedding itself
        # tmp = self.pos_emb(
        #         seq=inputs_embeds.shape[1], offset=past_key_values_length)
        # inputs_embeds = inputs_embeds + tmp
